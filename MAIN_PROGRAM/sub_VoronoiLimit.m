function [V,C,XY,VX_BC] = sub_VoronoiLimit(varargin)
% This subroutine calls a library that computes vertices’ coordinates of Voronoi 
% cells and indices of vertices within each Voronoi cell.
% --------------------------------------------------------------
...
% DETERMINE IF A MORE RECENT VERSION OF THE FUNCTION IS AVAILABLE ON THE MATHWORKS FILEEXCHANGE
try
    data = webread('http://se.mathworks.com/matlabcentral/fileexchange/34428-voronoilimit');
    ixVersion=strfind(data,'Version: ');
    data=data(ixVersion+9:ixVersion+80);
    ixsspan=strfind(data,'</span>');
    data=data(1:ixsspan(1)-1);
    ixp=strfind(data,'.');
    version_online=[str2double(data(1:ixp(1)-1)) str2double(data(ixp(1)+1:end))];
    update_available=0;
    if version_online(1)>version(1)
        update_available=1;
    else
        if version_online(2)>version(2)
            update_available=1;
        end
    end
    if update_available==1
        warndlg(['NOTE: A more recent version (ver. ',num2str(version_online(end,1)),'.',num2str(version_online(end,2)),') of the VoronoiLimit function is now available on the mathworks fileexchange. Currently running version ',num2str(version(end,1)),'.',num2str(version(end,2))])
    end
catch
...
% To ensure everytime the voronoi cell can be generated, the following
% perturbation is required
x_original = x;
y_original = y;
crs_original = crs;

% Giving slight perturbation so it can generate the voronoi cell without
% problem (the problem is due to the bad assignment of interior and boundary nodes)
x = x + sqrt(polyarea(crs(:,1),crs(:,2)))*1E-10*(rand(size(x))-0.5);
y = y + sqrt(polyarea(crs(:,1),crs(:,2)))*1E-10*(rand(size(y))-0.5);
crs = crs + sqrt(polyarea(crs(:,1),crs(:,2)))*1E-10*(rand(size(crs))-0.5);
...
% Tsung-Hui Huang, This expression is more accurate
mix=find(xix(2,:)==1)';
...
% Tsung-Hui Huang, 08262018
% We need the voronoi cell boundary vertices
VX_BC = [];
